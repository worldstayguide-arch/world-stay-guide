...
import type { PackageStatus, TravelPackageRecord } from '@/lib/packages';
...
const emptyDraft: PackageDraft = {
  ...
  alt: 'Pakistan mountain travel package',
  badge: 'Open Trip',
  status: 'Active',
  accommodation: '2N hotel stay',
  transport: 'Private transport',
  meals: 'Breakfast included',
};

function packageToDraft(item: TravelPackageRecord): PackageDraft {
  return {
    name: item.name,
    ...
    slotsLeft: String(item.slotsLeft),
    image: item.image,
    alt: item.alt,
    badge: item.badge ?? '',
    status: item.status,
    accommodation: item.features[0]?.value ?? '',
    transport: item.features[1]?.value ?? '',
    meals: item.features[2]?.value ?? '',
  };
}
...
export default function PackagesAdminPage() {
  ...
  useEffect(() => {
    ...
    async function loadPackages() {
      ...
      try {
        const response = await fetch('/api/packages?admin=1', { cache: 'no-store' });
        const data = (await response.json()) as { packages?: TravelPackageRecord[]; error?: string };
  ...
  const updateDraft = (field: keyof PackageDraft, value: string) => {
  ...
  const openEditor = (item: TravelPackageRecord) => {
    setSelectedId(item.id);
    setDraft(packageToDraft(item));
    setIsEditing(true);
    setMessage('');
  };
  ...
  const handleImageUpload = (file: File | undefined) => {
    ...
    reader.onload = () => {
      if (typeof reader.result === 'string') {
        ...
        updateDraft('alt', `${draft.name} travel package image`);
      }
    };
  ...
  const savePackage = async () => {
    ...
    const payload = {
      ...draft,
      slotsLeft: Number(draft.slotsLeft) || 0,
    };

    try {
      const response = await fetch(selectedId ? `/api/packages/${selectedId}` : '/api/packages', {
        method: selectedId ? 'PATCH' : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      const data = (await response.json()) as { package?: TravelPackageRecord; error?: string };
      ...
      setPackages((current) => {
        if (!selectedId) return [data.package!, ...current];
        return current.map((item) => (item.id === selectedId ? data.package! : item));
      });
      setSelectedId(data.package.id);
      setDraft(packageToDraft(data.package));
      setMessage('Package saved to database.');
      setIsEditing(false);
    } catch (error) {
  ...
  const deletePackage = async (id: number) => {
    ...
    try {
      ...
      setMessage('Package deleted from database.');
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#F5F5F7] text-neutral-900">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-neutral-200 bg-white/90 px-5 backdrop-blur md:px-8">
          <h1 className="text-lg font-semibold tracking-tight">Packages Management</h1>
          ...
          <section className="rounded-[1.75rem] bg-gradient-to-br from-neutral-950 via-[#143C46] to-[#0A5C8E] p-6 text-white shadow-xl shadow-neutral-900/10 md:p-8">
            <div className="flex flex-col justify-between gap-5 xl:flex-row xl:items-start">
              <div>
                ...
                <h2 className="text-3xl font-semibold tracking-tight md:text-4xl">Pick the Place Settings</h2>
                <p className="mt-2 max-w-xl text-sm text-white/70">
                  Add destinations, categories, prices, dates, images, and package details for the home page.
                </p>
              </div>
              <button
...
type PackageListProps = {
  ...
  isLoading: boolean;
  onEdit: (item: TravelPackageRecord) => void;
  onDelete: (id: number) => void | Promise<void>;
  onNew: () => void;
};

function PackageList({ packages, isLoading, onEdit, onDelete, onNew }: PackageListProps) {
  return (
    <section className="mt-10">
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        ...
        <div className="hidden items-center gap-2 rounded-2xl border border-neutral-200 bg-white px-4 py-2 md:flex">
          ...
          <input className="w-56 bg-transparent text-sm outline-none" placeholder="Search packages" />
        </div>
      </div>
      ...
      <div className="space-y-5">
        ...
        {!isLoading && packages.length === 0 && (
          <div className="rounded-2xl border border-dashed border-neutral-300 bg-white p-10 text-center shadow-sm">
            ...
            <p className="mt-2 text-sm text-neutral-500">Add the first package for the Pick the Place section.</p>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="mb-3 flex flex-wrap items-center gap-2">
                <Badge icon={MapPin}>{item.destination}</Badge>
                <Badge icon={Tag}>{item.category}</Badge>
                <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">
                  {item.status}
                </span>
              </div>
              <h3 className="truncate text-lg font-semibold">{item.name}</h3>
              <p className="mt-1 text-sm leading-relaxed text-neutral-500">{item.location}</p>
            </div>
            <div className="grid gap-1 text-sm font-semibold text-neutral-700 sm:grid-cols-3 xl:w-80">
              <span>{item.price}</span>
              <span>{item.dateLabel}</span>
              <span>{item.slotsLeft} slots</span>
            </div>
...
type PackageEditorProps = {
  draft: PackageDraft;
  selectedPackage?: TravelPackageRecord;
  isSaving: boolean;
  onBack: () => void;
...
function PackageEditor({ draft, selectedPackage, isSaving, onBack, onChange, onImageUpload, onSave }: PackageEditorProps) {
  return (
    <section className="mt-10">
      <div className="mb-6 flex flex-col justify-between gap-4 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm xl:flex-row xl:items-center">
        ...
        >
          ...
          {isSaving ? 'Saving...' : selectedPackage ? 'Update' : 'Create'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="grid grid-cols-1 gap-5 rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm md:grid-cols-2">
          <Field label="Package Name" value={draft.name} onChange={(value) => onChange('name', value)} />
          <Field label="Destination" value={draft.destination} onChange={(value) => onChange('destination', value)} />
          ...
          <Field label="Price" value={draft.price} onChange={(value) => onChange('price', value)} />
          <Field label="Date" value={draft.dateLabel} onChange={(value) => onChange('dateLabel', value)} />
          <Field label="Trip Type" value={draft.tripType} onChange={(value) => onChange('tripType', value)} />
          <Field label="Slots Left" value={draft.slotsLeft} type="number" onChange={(value) => onChange('slotsLeft', value)} />
          <Field label="Badge" value={draft.badge} onChange={(value) => onChange('badge', value)} />
          ...
          <Field label="Accommodation" value={draft.accommodation} onChange={(value) => onChange('accommodation', value)} />
          <Field label="Transport" value={draft.transport} onChange={(value) => onChange('transport', value)} />
          <Field label="Meals" value={draft.meals} onChange={(value) => onChange('meals', value)} />
          <Field label="Image Alt Text" value={draft.alt} onChange={(value) => onChange('alt', value)} />
        </div>
        ...
        <aside className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
          ...
          <div className="relative mb-3 h-44 overflow-hidden rounded-xl bg-neutral-100">
            <Image src={draft.image} alt={draft.alt} fill sizes="320px" className="object-cover" unoptimized={draft.image.startsWith('data:')} />
          </div>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
            <Info icon={Users} label="Trip Type" value={draft.tripType} />
            <Info icon={Calendar} label="Date" value={draft.dateLabel} />
          </div>
        </aside>
      </div>
    </section>
...
function Info({ icon: Icon, label, value }: { icon: typeof Users; label: string; value: string }) {
  return (
    <div className="rounded-xl bg-neutral-50 p-3">
      <Icon className="mb-2 h-4 w-4 text-[#0A5C8E]" />
      <div className="text-[10px] font-bold uppercase text-neutral-400">{label}</div>
      <div className="truncate text-xs font-semibold text-neutral-800">{value}</div>
    </div>
  );
}
